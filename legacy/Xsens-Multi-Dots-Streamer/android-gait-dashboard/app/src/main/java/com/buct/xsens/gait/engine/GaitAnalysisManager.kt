...
class GaitAnalysisManager(private val context: Context) {
    ...
    fun analyzeGait(filePath: String, weightStr: String): String {
        return analyzeGait(filePath, weightStr, "-1.0", "-1.0")
    }
    ...
    fun analyzeGait(filePath: String, weightStr: String, startTimeStr: String, endTimeStr: String): String {
        ...
        android.util.Log.d("GaitAnalysis", "Analyzing file: $filePath, weight: $weight, start: $startTimeS, end: $endTimeS")
        return try {
            val py = Python.getInstance()
            val module = py.getModule("gait_analyzer")
            // python: process_gait_data(file_path, weight_kg, start_time_s=-1.0, end_time_s=-1.0)
            val result = module.callAttr("process_gait_data", filePath, weight, startTimeS, endTimeS)
    ...
    fun analyzeGaitContent(content: String, weightStr: String): String {
        return analyzeGaitContent(content, weightStr, "-1.0", "-1.0")
    }
    ...
    fun analyzeGaitContent(content: String, weightStr: String, startTimeStr: String, endTimeStr: String): String {
        ...
        return try {
            ...
            analyzeGait(tempFile.absolutePath, weightStr, startTimeStr, endTimeStr)
        } catch (e: Exception) {
