...
# =============================================================================
LOW_CUTOFF_HZ         = 6      # 低通滤波截止频率 (Hz)
MSW_WINDOW_MS         = 300    # MSW 峰值检测半窗 (ms)
TC_OFFSET_AFTER_IC_MS = 50     # IC 后多少 ms 开始搜 TC
FS_OFFLINE            = 120    # 离线数据采样率 (Hz)
FS_ONLINE             = 60     # 在线数据采样率 (Hz)
...
#   对应 gait_analyzer.py 的 gait_identification_60hz()
# =============================================================================
def gait_identification(idata, fs=120):
    """
    步态事件识别（与 gait_analyzer.gait_identification_60hz 等价）。
...
def preprocess(file_path):
    ...
    返回包含 Timestamp / ACC.X/Y/Z / Gmax(°/s) / Gyro.X/Y/Z 的 DataFrame，
    以及 (fmt, fs) 格式标识和采样率。
    """
    # 找到真正的表头行
    skip = 0
    ...
    fs  = FS_OFFLINE if fmt == 'offline' else FS_ONLINE
...
def plot_results(idata_proc, fmt, fs, file_name):
    ...
    fig.suptitle(
        f"步态事件  |  {file_name}  |  {fmt}  {fs}Hz  "
        f"低通={LOW_CUTOFF_HZ}Hz  MSW窗={MSW_WINDOW_MS}ms",
...
def main():
    ...
    # 2. 预处理
    print(f"\n正在处理: {local_file}")
    idata_raw, fmt, fs = preprocess(local_file)
    print(f"格式={fmt}  采样率={fs}Hz  总行数={len(idata_raw)}")
    ...
    print("运行步态事件识别算法...")
    HS, TO, MS, idata_proc = gait_identification(idata, fs=fs)
    ...
    plot_results(idata_proc, fmt, fs, file_name)
